...
res <- read.csv(paste(outputdir, "/quiz-", usercount, ".out", sep=""), header = FALSE)
colnames(res) <- header
...
filplotdraw("GET: Login", paste(outputdir, "/quiz-", usercount, "-getlogin.svg", sep=""))
filplotdraw("POST: Login", paste(outputdir, "/quiz-", usercount, "-postlogin.svg", sep=""))
filplotdraw("GET: Quiz page", paste(outputdir, "/quiz-", usercount, "-quizpage.svg", sep=""))
filplotdraw("POST: Start quiz", paste(outputdir, "/quiz-", usercount, "-startquiz.svg", sep=""))

# Questions based on qcount arg
for (i in 0:(qcount-1))
{
	grepby <- paste("POST: Answer question \\(", i, sep="")
	outpath <- paste(outputdir, "/quiz-", usercount, "-answerq", i, ".svg", sep="")
	filplotdraw(grepby, outpath)
}

filplotdraw("POST: Answer last question", paste(outputdir, "/quiz-", usercount, "-answerq", qcount, ".svg", sep=""))
filplotdraw("POST: Finish Quiz", paste(outputdir, "/quiz-", usercount, "-finishquiz.svg", sep=""))


# Draw summery bar chart
svg(paste(outputdir, "/quiz-", usercount, "-summary.svg", sep=""), width=12, height=8)
